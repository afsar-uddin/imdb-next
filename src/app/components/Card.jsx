  ...
  return (
    <div className='group m-1 cursor-pointer sm:hover:shadow-slate-400 sm:shadow-md rounded-lg sm:border sm:border-slate-400 sm:m-2  transition-shadow-200'>
        <Link href={`/movie/${result.id}`} className=''>
            <Image src={`https://image.tmdb.org/t/p/original/${result.backdrop_path || result.poster_path}`} width={500} height={300} className='sm:rounded-t-lg group-hover:opacity-65 transition-opacity duration-300' alt={result.title}></Image>
